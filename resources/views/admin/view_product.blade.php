...
@section('content')
    <div class="pb-20 flex flex-col px-4 md:px-8 xl:px-16">
        <div class="sticky top-24 z-[100] pb-5 left-24 w-full bg-white">
            @php
                $dynamicHref = '';
                
                switch ($product->product_type) {
                    case 'simple_cake':
                        $dynamicHref = '/admin/simple-cake';
                        break;
                    case 'custom_cake':
                        $dynamicHref = '/admin/custom-cake';
                        break;
                    case 'pastry':
                        $dynamicHref = '/admin/pastry';
                        break;
                    // Add more cases for other product types if needed
                    default:
                        $dynamicHref = '#'; // Default to a hash symbol if the type is not recognized
                        break;
                }
            @endphp
            <x-button element="anchor" href="{{ $dynamicHref }}" style="primary"
                class="relative -top-[100%] flex-row space-x-3 font-dm flex rounded-lg max-w-fit !text-base">
                <i class="fa-solid fa-arrow-left inline-flex"></i>
                <p class="inline-flex">Back
                    to
                    Admin</p>
            </x-button>
        </div>
